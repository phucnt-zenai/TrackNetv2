...
def process_video(video_file, gt_csv, model, num_frame, batch_size, save_dir, input_type='3d', tolerance=4):
    ...
    out_csv_file = f'{save_dir}/{folder_name}_{video_name}_ball.csv'
    ...
    success = True
    frame_count = 0  # Count variable to manage frame_queue size per iteration

    total_TP, total_TN, total_FP1, total_FP2, total_FN = 0, 0, 0, 0, 0
    total_frame_time = 0  # Total time per frame

    # Read frames
    while success:
        frame_queue = []
        for _ in range(num_frame * batch_size):  # 3*1 = 3 frames per batch
        ...
        if len(frame_queue) % num_frame != 0:
            frame_queue = []
            # Reset frame pointer and re-read
        ...
        # Stack frames for input: (B, H, W, 3*F)
        x = get_frame_unit(frame_queue, num_frame)

        # === INFERENCE AND POST-PROCESSING ===
        coords_list = []  # Store prediction results for next loop
    ...
    print(f"[✓] Finished processing video {video_file}, average time per frame: {avg_frame_time:.4f} seconds")
...
def process_all_videos(data_dir, model, num_frame, batch_size, save_dir, tolerance=4):
    ...
    total_frame_time_all = 0  # Total time for all frames
    total_frame_count = 0     # Total number of frames processed
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/kaggle/input/badminton-testset', help='Folder contains match1/, match2/, ...')
    ...
    process_all_videos(args.data_dir, model, args.num_frame, args.batch_size, args.save_dir, tolerance = args.tolerance)
